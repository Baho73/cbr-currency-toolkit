...
logger.info("[ModuleName][function_name][BLOCK_NAME] message",
            extra={"correlation_id": correlation_id, "stable_field": value})
```
...
task1-currency-web/      - FastAPI веб-конвертер валют (бэкенд + фронтенд)
task2-cbr-cli/           - Асинхронный CLI-аналитик курсов + Docker
task3-apps-script/       - Google Apps Script для Google Таблицы
```
...
You MUST never assume a scope is fully handled, or report a task as "done", without both of the
following having exited successfully in the same session:

- the project's test command (`pytest` in each Python sub-project)
- `grace lint --path .`

"The code looks right" is not evidence. "I ran the tests and they passed, and grace lint exited 0"
is evidence. If you cannot run one of them (no runtime, missing dependency), say so explicitly —
